---
title: Longfei Ma

bio: My research focuses on exploring the quantitative relationship between rice chlorophyll and spectrum, and estimating chlorophyll content based on drone multispectral and hyperspectral data.

first_name: Longfei
last_name: Ma

education:
  courses:
  - course: PhD in Resources and Environment
    institution: Wuhan University
    year: 2022-2026
  - course: MSc in Crop Cultivation and Tillage
    institution: China Agriculture University
    year: 2019-2022
  - course: BSc in Tobacco Science
    institution: Henan Agricultural University
    year: 2015-2019

interests:
- Chlorophyll content estimation
- Multi spectral Remote Sensing
- Precision Agriculture

organizations:
- name: Wuhan University
role: Researcher, Wuhan University

social:
- icon: envelope
  icon_pack: fas
  link: mailto:longfei.ma@tum.de
- icon: twitter
  icon_pack: fab
  link: "#"
- icon: google-scholar
  icon_pack: ai
  link: https://scholar.google.com.hk/citations?user=MaB4QRAAAAAJ&hl=zh-CN
- icon: github
  icon_pack: fab
  link: "#"

superuser: false
user_groups:
- Alumni
---
Longfei Ma is a visiting researcher at the Precision Agriculture Laboratory of the Technical University of Munich (TUM). His current research centers on the accurate estimation of chlorophyll content in rice at both the leaf and canopy scales based on spectral data. His work involves the exploration of leaf absorption spectral characteristics and the application of canopy spectral invariant theory.
